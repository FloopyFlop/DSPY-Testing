...
class FeedbackGenerator(dspy.Signature):
    """Generate SAVAGE, brutally honest, hilarious roasts with actionable technical feedback.
...
class FeedbackEvaluator(dspy.Signature):
    """Evaluate feedback based on how BRUTALLY HONEST, SAVAGE, and HILARIOUS it is while staying technically accurate.

    High scores should go to feedback that:
    - Roasts the code mercilessly with sarcasm and humor
    - Uses creative analogies and comparisons
    - Is technically accurate but delivers criticism like a comedian
    - Makes you laugh while teaching you something
    - Doesn't hold back or sugarcoat anything

    Low scores for:
    - Polite, corporate, boring feedback
    - Generic advice without personality
    - Missing opportunities to roast obvious mistakes
    """

    original_content = dspy.InputField(desc="The code/idea that was reviewed")
    feedback = dspy.InputField(desc="The feedback that was given")

    savagery_score = dspy.OutputField(desc="1-10: How savage and brutally honest? (10 = absolutely ruthless roast)")
    humor_score = dspy.OutputField(desc="1-10: How funny and entertaining? (10 = hilarious, makes you laugh)")
    technical_score = dspy.OutputField(desc="1-10: How technically accurate? (10 = correct advice despite the roasting)")
    critique = dspy.OutputField(desc="How could this feedback be MORE savage and funnier while staying accurate?")
...
class LLMAsJudge:
    ...
    def __call__(self, example, prediction, trace=None) -> float:
        ...
        - Savagery (how brutally honest and ruthless)
        - Humor (how funny and entertaining)
        - Technical accuracy (still needs to be correct)
        """
        ...
        try:
            ...
            savagery = self._parse_score(eval_result.savagery_score)
            humor = self._parse_score(eval_result.humor_score)
            technical = self._parse_score(eval_result.technical_score)

            # Weighted average: HEAVILY emphasize savagery and humor!
            score = (0.5 * savagery + 0.3 * humor + 0.2 * technical) / 10.0
